...
> **2 Ağustos 2026 güncellemesi — tarihsel belge:** Aşağıdaki 13 Temmuz gövdesi ilk denetimin değiştirilmeyen tarihsel kaydıdır; güncel mimari veya release kararı olarak kullanılmamalıdır. Kanonik sistem artık React/Vite + Express + PostgreSQL 17 forced RLS + internal-auth FastAPI topolojisidir. Güncel doğrulamada kök paket 92 test, temiz PostgreSQL üzerinde 39 Express entegrasyon + 2 RLS testi, FastAPI 73 test ve Ruff, .NET referans 15 test, Next referans type-check/build, şifreli backup/restore ve Trivy 0.72 HIGH/CRITICAL image taramaları başarılıdır. Güncel karar ve kalan release engelleri için `PRODUCTION_READINESS_REPORT.md`; kurulum için `DEPLOYMENT.md`; mimari için `ARCHITECTURE.md` esas alınmalıdır.
...
- İsteğe bağlı NVIDIA DeepSeek veya Gemini entegrasyonu ile veri/doküman destekli sohbet yapmak
...
Node.js container :3010
  ...
  - ETL ve rapor mantığı
        ...
        |
        +------> NVIDIA DeepSeek / Gemini (opsiyonel, anahtar + açık veri izni gerekir)
        |
...
| Opsiyonel AI | NVIDIA NIM/OpenAI uyumlu chat completions, Google Gemini SDK |
...
- Harici AI çağrısı için seçili sağlayıcı anahtarı (`NVIDIA_API_KEY` veya `GEMINI_API_KEY`) ve `ALLOW_EXTERNAL_AI_DATA=true` birlikte gerekir.
...
| Orta | Harici AI anahtarı zorunlu başlangıç koşuluydu | Opsiyonel entegrasyon yoksa tüm uygulama başlayamıyordu. |
...
Gerçek e-posta, SMS, ödeme veya ücretli AI çağrısı yapılmadı. Harici NVIDIA/Gemini ve müşteri REST servisi, gerçek güvenli anahtar/izin sağlanmadığı için güvenli devre dışı durum ve yerel doğrulamalar üzerinden değerlendirildi.
