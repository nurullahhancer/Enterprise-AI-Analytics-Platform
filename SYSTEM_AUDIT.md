...
Sistem kod ve container yapılandırması açısından production adayıdır. Müşteri erişimine açılmadan önce sunucu sahibinin benzersiz secret'ları sağlaması, ilk yönetici hesabını oluşturması ve domain kullanılıyorsa DNS/TLS işlemlerini tamamlaması zorunludur.
...
Kullanıcı tarayıcısı
        |
        | HTTPS (önerilen) / HTTP (yalnızca ilk IP testi)
        v
...
|---|---:|---:|---|---|
| `app` | 3010 | `${APP_PORT:-3000}` | `edge`, `backend` | `app-data:/app/data` |
| `ml-service` | 8000 | Yayınlanmaz | yalnızca internal `backend` | Model cache'i geçici |

Uygulama portu varsayılan olarak `0.0.0.0:3000` üzerinde yayınlanır. Reverse proxy kullanıldığında `APP_BIND_IP=127.0.0.1` olmalıdır.
...
| Dışa açık port | Yalnızca kanonik app portu; ML internal |
...
- Domain olmadan HTTPS/HSTS koruması tam olarak sağlanamaz.
...
| Production runtime smoke | Kısmi/öngörülen fail-closed | Public ana sayfa 200; DB+ML ok; gerçek JWT secret olmadığı için health HTTP 503/degraded ve app unhealthy |
...
4. Domain kullanılacaksa DNS kaydı ve TLS sertifikası kurulmalı; app yalnızca localhost'a bind edilmelidir.
...
- Domain bildirilmediği için ana erişim modeli web smoke testi için `http://45.133.36.77:3000` olarak kabul edildi; gerçek login/müşteri verisi için TLS zorunlu sayıldı.
...
- Kök production: `Dockerfile`, `docker-compose.yml`, `.dockerignore`, `.env.example`
...
Kritik sahte davranışlar ve auth/RBAC/SSRF/secret/port açıklıkları giderilmiş, kanonik uygulama belirlenmiş, production container sertleştirilmiş ve ana iş akışları otomatik testlerle doğrulanmıştır. Kanonik Compose yığını VDS'de çalışmaktadır: public ana sayfa `http://45.133.36.77:3000/`, ML yalnız internal ağdadır. Gerçek `.env`/secret olmadığı için app bilinçli olarak HTTP 503/degraded ve `unhealthy`; login kapalıdır.

Sistem şu şartlar tamamlanmadan “müşteriye açık ve tam teslim” sayılmamalıdır: gerçek secret'ların sağlanması, token'lı ilk admin kaydı, gerçek kullanıcı trafiği için domain/TLS, yedekleme takvimi ve kimlikli dış erişim smoke testi. Ayrıntılı operasyon adımları `DEPLOYMENT.md` içindedir.
