...
router.post('/analyses/:id/interpret', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const evidence = {
      ...
      availableColumns: Array.isArray(result.feature_columns) ? result.feature_columns.slice(0, 100) : [],
      forecastQuality: result.forecast?.metrics?.quality_grade ?? null,
      forecastDecisionSupport: result.forecast?.metrics?.decision_support ?? null,
    ...
    const prompt = `Sen, veri ve işletme yönetimi konusunda uzman kıdemli bir iş danışmanısın. Aşağıdaki kanıt doğrulanmış analiz sonuçlarını içerir. Türkçe, doğal, profesyonel, zengin ve uygulanabilir bir dille yaz.\n\nYanıt biçimi şöyle olsun:\n## 📊 Genel Değerlendirme ve Beklenen Sonuçlar\nÖnümüzdeki dönemde ne kadarlık bir hacim/ciro beklendiğini ve verinin genel gidişatını net sayılarla özetle. businessSummary.useTotal true ise expectedTotal değerini, false ise expectedPeriodAverage değerini vurgula.\n\n## 📈 Trend ve Değişim Analizi\nİlk dönem ile son dönem arasındaki artış/düşüş görünümünü sayıyla açıkla. Varsa olası alt ve üst aralığı belirt.\n\n## 💡 Stratejik Aksiyon Önerileri\nİşletmenin hemen uygulayabileceği somut ve veriye dayalı stratejik öneriler yaz. Veri kalitesi skoru düşükse (very_low veya low) operasyonel kararlardan önce veri toplama ve izlemeyi öner.\n\n## ⚠️ Riskler ve Tahmin Notu\nTahminin geçmiş verilere dayandığını, dış etkenleri ve dikkat edilmesi gereken noktaları özetle.\n\nKurallar:\n- Verideki metrikleri ve sayıları doğru temel al; işletme için değer katacak zengin iş yorumları yap.\n- Ham teknik terimler (MAE, RMSE, R², ROC-AUC, holdout) veya kod/JSON isimleri yerine anlaşılır iş dili kullan.\n- Kalite puanını doğru çıkma olasılığı olarak sunma.\n\n<dogrulanmis_analiz>\n${JSON.stringify(evidence)}\n</dogrulanmis_analiz>`;
